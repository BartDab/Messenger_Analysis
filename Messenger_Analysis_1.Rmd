...
```{r}
data1<-jsonlite::fromJSON(txt='E:/data_10052020/messages/inbox/FaniPAWLA_wq9QgA1Q8g/data1.txt')
...
ggplot(df2, aes(x=Date,y=n))+
  geom_col(color='darkolivegreen3')+
  scale_x_datetime(date_breaks="1 month",date_labels="%m-%Y")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  ggtitle('Aktywność Fanów Pawła ❤🎸')+
...
data_no_emoji$text<-chartr(paste(names(unwanted_array), collapse=''),
         ...
         data_no_emoji$text)
nanotek<-dplyr::filter(data_no_emoji, grepl('Ania', text))
...
custom_stop<-c('tez','bym','maja','wie','zalacznik','miec')
```
...
wordcloud(names(freq),freq,max.words=30,color=cividis(1),main='Title')
```
...
```{r}  
  animated = staticplot + transition_states(Date, transition_length = 4, state_length = 2) +
    ...
    labs(title = 'Fani Pawła  ❤🎸: {closest_state}',  
...
#  animate(animated, 1400, fps = 20,  width = 1200, height = 1000, 
  ...
  video<-animate(animated, 600, fps = 25,  width = 1200, height = 1000,
          renderer = ffmpeg_renderer())
  anim_save("top_spamers.mp4", animation = video)
```