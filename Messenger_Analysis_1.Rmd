...
```{r}
data1<-jsonlite::fromJSON(txt='E:/data18052020/messages/inbox/AniaSmoter_nhbNF6vGSw/data1.txt')
...
ggplot(df9, aes(x = hour(Time))) + geom_histogram(bins=24,breaks = seq(0, 
  ...
  theme(plot.title = element_text(hjust = 0.5))+
    scale_x_continuous("", limits = c(0, 24), breaks = seq(0, 24), labels = seq(0, 
        24))
#definitely not my type of plot
```
...
wek<-c('hej','hej','hejoo')
data_no_emoji$text<-gsub(paste(wek, collapse = '|'), 'hej', data_no_emoji$text)
wek<-c('Halkooooo','haloo')
data_no_emoji$text<-gsub(paste(wek, collapse = '|'), 'halo', data_no_emoji$text)
data_no_emoji$text<-gsub('Aniu', 'Ania', data_no_emoji$text)
data_no_emoji$text<-gsub('Baaartooooosz', 'Bartosz', data_no_emoji$text)
